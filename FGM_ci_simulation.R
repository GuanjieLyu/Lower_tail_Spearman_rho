## FGM confidence interval simulation
##
...
## thresholds. The resulting figures compare the average bootstrap confidence
## interval endpoints with empirical Monte Carlo sampling intervals computed
## from the simulated estimator values.
##
...
results_dir <- sprintf("fgm_ci_bootstrap_mc_results_%s", timestamp)
...
cat("FGM Bernstein CI simulation\n")
...
ci_summary <- aggregate(
  cbind(rho_hat, lower, upper, width, half_width,
...
mc_interval <- do.call(
  ...
  lapply(
    split(raw_results,
          list(raw_results$theta, raw_results$n, raw_results$m, raw_results$p),
          drop = TRUE),
    function(dat) {
      data.frame(
        theta = dat$theta[1],
        ...
        m = dat$m[1],
        p = dat$p[1],
        mc_lower = unname(quantile(dat$rho_hat, probs = alpha / 2,
                                   type = 8, na.rm = TRUE)),
        mc_upper = unname(quantile(dat$rho_hat, probs = 1 - alpha / 2,
                                   type = 8, na.rm = TRUE)),
        mc_sd = sd(dat$rho_hat)
      )
    }
  )
)
...
plot_case <- function(dat, theta, n, figure_dir) {
  dat <- dat[order(dat$p), ]
  ...
  lines(dat$p, dat$mc_lower, lwd = 5, col = "#2ca02c", lty = 4)
  lines(dat$p, dat$mc_upper, lwd = 5, col = "#2ca02c", lty = 4)
...
for (i in seq_len(nrow(case_keys))) {
  ...
  plot_case(dat_i, theta_i, n_i, figure_dir)
}
